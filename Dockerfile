...
ENV LD_LIBRARY_PATH="${LD_LIBRARY_PATH}:/usr/lib/llvm-20/lib/clang/20/lib/linux/"
...
RUN true \
	...
	&& apt install -y --no-install-recommends \
		...
		libpcre2-dev libargon2-dev libedit-dev libsodium-dev llvm-20 libonig-dev \
		gdb libcapstone-dev

RUN git clone https://github.com/php/php-src -b PHP-8.5 --depth 1 && cd php-src \
    && ./buildconf \
    ...
    && export LDFLAGS='-g -fsanitize=address -shared-libasan -Wl,-rpath=/usr/lib/llvm-20/lib/clang/20/lib/linux/' \
    \
	&& make -j100 \
	&& make install
